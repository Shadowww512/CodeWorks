...
public class MoviesStats {
	...
	public static long problem2_countMoviesThatStartWith(Stream<Movie> movies, String startsWith) {
		return movies.filter(m -> m.getTitle().startsWith(startsWith)).count();
	}
	...
	public static String problem3_getMoviesThatContainString(Stream<Movie> movies, String str) {
		Stream<String> titles = movies.filter(m -> m.getTitle().toLowerCase().contains(str.toLowerCase()))
									  .map(m -> m.getTitle());
		return titles.collect(Collectors.joining(", "));
	}
	...
	public static Map<Integer, Long> problem4_countMoviesByYear(Stream<Movie> movies, int minYear,
			int maxYear) {
		Stream<Movie> withinRange = movies.filter(m -> (m.getYear() <= maxYear) && (m.getYear() >= minYear));
		return withinRange.collect(Collectors.groupingBy(m -> m.getYear(),TreeMap::new,Collectors.counting()));
	}
	...
	public static long problem6_countActors(Stream<Movie> movies) {
		...
		for (List<String> e : movies.map(m -> m.getActors()).collect(Collectors.toList())){
			all.addAll(e);
		}
	...
	public static long problem6_countDirectors(Stream<Movie> movies) {
		List<String> all = new ArrayList<>();
		for (List<String> e : movies.map(m -> m.getDirectors()).collect(Collectors.toList())){
			all.addAll(e);
		}
		return all.stream().distinct().count();
	}
	...
	public static long problem6_countProducers(Stream<Movie> movies) {
		List<String> all = new ArrayList<>();
		for (List<String> e : movies.map(m -> m.getProducers()).collect(Collectors.toList())){
			all.addAll(e);
		}
		return all.stream().distinct().count();
	}
	...
	public static double problem7_averageNumActors(Stream<Movie> movies) {
		IntStream numOfActors = movies.mapToInt(m -> m.getActors().size());
		return numOfActors.average().orElse(0);
	...
	public static double problem7_averageNumDirectors(Stream<Movie> movies) {
		IntStream numOfDirectors = movies.mapToInt(m -> m.getDirectors().size());
		return numOfDirectors.average().orElse(0);
	}
	...
	public static double problem7_averageNumProducers(Stream<Movie> movies) {
		IntStream numOfProducers = movies.mapToInt(m -> m.getProducers().size());
		return numOfProducers.average().orElse(0);
	}
	...
	public static List<Movie> problem8_moviesWithNWordsInTitle(Stream<Movie> movies, int n) {
		return movies.filter(m -> m.getTitle().split(" ").length == n).collect(Collectors.toList());
	}
	...
	public static Optional<Movie> problem9_oldestMovie(Stream<Movie> movies) {
		return movies.filter(m -> m.getYear() != 0).min((a,b)-> a.getYear() - b.getYear());
	}
	...
	public static Optional<Movie> problem10_newestMovie(Stream<Movie> movies) {
		return movies.max((a,b) -> a.getYear() - b.getYear());
	}
}